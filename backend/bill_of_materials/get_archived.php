...
$deletedChildrenQuery = "
    SELECT DISTINCT d.PART_SURROGATE
    FROM details_tb d
    JOIN material_tb m 
...
";
$deletedChildrenResult = $bomMysqli->query($deletedChildrenQuery);
$deletedChildren = [];
while ($row = $deletedChildrenResult->fetch_assoc()) {
    $deletedChildren[] = $row['PART_SURROGATE'];
}
$deletedChildrenList = !empty($deletedChildren)
    ? "'" . implode("','", $deletedChildren) . "'"
    : "''";
...
SELECT 
    ...
    CASE WHEN d.ROW_TYPE = 0 THEN p.DELETE_STATUS ELSE m.DELETE_STATUS END AS DELETE_STATUS,
    d.PROCESS,
    d.CLASS,
    d.SUPPLIER,
    d.QTY,
    ...
    CASE WHEN d.ROW_TYPE = 0 THEN p.DELETED_AT ELSE m.DELETED_AT END AS ARCHIVED_DATE,
    p.RID AS PART_ID,
    m.RID AS MATERIAL_ID,
    d.ROW_TYPE,
    p.DELETE_STATUS AS PART_DELETE_STATUS
FROM details_tb d
...
LEFT JOIN user_tb u 
    ON (d.ROW_TYPE = 0 AND p.DELETED_BY = u.RFID)
    OR (d.ROW_TYPE = 1 AND m.DELETED_BY = u.RFID)
WHERE 
    (
        (d.ROW_TYPE = 0 AND p.DELETE_STATUS = 1) OR
        (d.ROW_TYPE = 1 AND m.DELETE_STATUS = 1 AND p.DELETE_STATUS = 0) OR
        (d.ROW_TYPE = 1 AND p.DELETE_STATUS = 1) OR
        (d.ROW_TYPE = 0 AND p.DELETE_STATUS = 0 AND p.PART_SURROGATE IN ($deletedChildrenList))
    )
ORDER BY p.CUSTOMER, p.PART_SURROGATE, m.MATERIAL_SURROGATE
";
